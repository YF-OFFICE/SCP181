        ...
        public void Hurt(PlayerHurtingEventArgs ev)
        {
            if (ev.Player.PlayerId == SCP181ID)
            {
                if (ev.Player != null && ev.Attacker != null)
                {
                    ...
                    {
                        ev.IsAllowed = false;
                        ev.Player.SendHint("你幸运地躲避了一次伤害");
                        ev.Attacker.SendHint("你很倒霉 没有伤到181");
                ...
                case "set":
                    ...
                    else
                    {
                       ...
                       var  player = Player.Get(id);             
                        ...
                        player.MaxHealth = Plugin1.plugin.Config.Health;
                        player.Health = player.MaxHealth;
                        player.GroupName = "SCP181";
                        player.GroupColor = "yellow";
                        player.ClearInventory();
                        if (!Plugin1.plugin.Config.itemTypes.IsEmpty())
                        {
                            Plugin1.plugin.Config.itemTypes.ForEach(x => player.AddItem(x));
                        }
                        player.ClearBroadcasts();
                        player.SendBroadcast($"你是SCP181\n具有{Plugin1.plugin.Config.Luck}%概率打开门 {Plugin1.plugin.Config.Luck1}%免伤 背包里还有好东西", 5);
